...
if (isset($_FILES["avatar"]["name"]) && $_FILES["avatar"]["tmp_name"] != ""){
	...
	$avatar = $row[0];
	if($avatar != ""){
		$picurl = "/v2/app/dashboard/userdata/$log_username/$avatar"; 
	    if (file_exists($picurl)) { unlink($picurl); }
	}
	$moveResult = move_uploaded_file($fileTmpLoc, "userdata/$log_username/$db_file_name");
	if ($moveResult != true) {
		header("location: message.php?msg=ERROR: File upload failed");
		exit();
	}
	include_once("php_includes/image_resize.php");
	$target_file = "userdata/$log_username/$db_file_name";
	$resized_file = "userdata/$log_username/$db_file_name";
	$wmax = 70;
	$hmax = 70;
	...
	$query = mysqli_query($db_conx, $sql);
	mysqli_close($db_conx);
	header("location: user/$log_username#tab_gallery");
	exit();
}
?><?php 
if (isset($_FILES["photo"]["name"]) && isset($_POST["gallery"])){
	...
	$query = mysqli_query($db_conx, $sql);
	mysqli_close($db_conx);
	header("location: /v2/app/dashboard/user/$log_username#tab_gallery");
	exit();
}
?><?php 
if (isset($_POST["delete"]) && $_POST["id"] != ""){
	...
	if($user == $log_username){
		$picurl = "userdata/$log_username/$filename"; 
