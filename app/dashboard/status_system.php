...
if (isset($_POST['action']) && $_POST['action'] == "status_post"){
	...
	// added this
	if($data == "||na||" && $image != "na"){
		$data = '<img src="/v2/app/dashboard/permUploads/'.$image.'" />';
		//$data = '<img src="permUploads/'.$image.'" class="biggerStatusImage" />';
	} else if ($data != "||na||" && $image != "na"){
		$data = $data.'<br /><img src="/v2/app/dashboard/permUploads/'.$image.'" />';
	...
	for($i = 0; $i < count($friends); $i++){
		$friend = $friends[$i];
		$app = "Status Post";
		$note = $log_username.' posted on: <br /><a href="/v2/app/dashboard/user/'.$account_name.'#status_'.$id.'">'.$account_name.'&#39;s Profile</a>';
		mysqli_query($db_conx, "INSERT INTO notifications(username, initiator, app, note, date_time, id) VALUES('$friend','$log_username','$app','$note',now(), '$id')");
...
if (isset($_POST['action']) && $_POST['action'] == "status_reply"){
	...
	$sql = "INSERT INTO status(osid, account_name, author, type, data, postdate)
	        VALUES('$osid','$account_name','$log_username','b','$chopit',now())";
	$query = mysqli_query($db_conx, $sql);
	$id = mysqli_insert_id($db_conx);
	...
	while ($row = mysqli_fetch_array($query, MYSQLI_ASSOC)) {
		...
		$app = "Status Reply";
		$note = $log_username.' commented here:<br /><a href="/v2/app/dashboard/user/'.$account_name.'#status_'.$osid.'">Click here to view the conversation</a>';
		mysqli_query($db_conx, "INSERT INTO notifications(username, initiator, app, note, date_time, id)
		             VALUES('$participant','$log_username','$app','$note',now(), '$id')");
	}
	mysqli_close($db_conx);
	echo "reply_ok|$id";
	exit();
...
if (isset($_POST['action']) && $_POST['action'] == "delete_status"){
    ...
    if ($author == $log_username || $account_name == $log_username) {
		...
		mysqli_query($db_conx, "UPDATE notifications SET did_read='1'WHERE id='$statusid' ");
        mysqli_close($db_conx);
	    echo "delete_ok";
		exit();
	}
...
if (isset($_POST['action']) && $_POST['action'] == "delete_reply"){
    ...
    if ($author == $log_username || $account_name == $log_username) {
		...
		mysqli_query($db_conx, "UPDATE notifications SET did_read='1' WHERE id='$replyid'");
		mysqli_close($db_conx);
	    echo "delete_ok";
		exit();
	}
...
if (isset($_POST['action']) && $_POST['action'] == "share"){
	...
	$row = mysqli_fetch_row($query);
	$data = "<b>Shared post by: $row[0]</b><hr>";
	$data .= $row[1];
